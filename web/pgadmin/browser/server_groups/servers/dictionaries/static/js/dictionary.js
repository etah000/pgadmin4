...
//
//////////////////////////////////////////////////////////////
define('pgadmin.node.dictionary', [
  ...
  if (!pgBrowser.Nodes['coll-dictionary']) {
    pgBrowser.Nodes['coll-dictionary'] =
  ...
  if (!pgBrowser.Nodes['dictionary']) {
    pgBrowser.Nodes['dictionary'] = pgBrowser.Node.extend({
      parent_type: 'server',
      type: 'dictionary',
      sqlAlterHelp: 'sql-alterdictionary.html',
      sqlCreateHelp: 'sql-createdictionary.html',
      ...
      callbacks: {
        /* Move objects from one dictionary to another */
        move_objects: function(args){
          ...
          var objModel = Backbone.Model.extend({
            idAttribute: 'id',
            defaults: {
              new_dicspc: undefined,
              obj_type: 'all',
              user: undefined,
            },
            schema: [{
              id: 'dicspc', label: gettext('New dictionary'),
              type: 'text', disabled: false, control: 'node-list-by-name',
              node: 'dictionary', select2: {allowClear: false},
              filter: function(o) {
          ...
          if(!Alertify.move_objects_dlg) {
            Alertify.dialog('move_objects_dlg' ,function factory() {
              return {
                main: function() {
                  var title = gettext('Move objects to another dictionary');
                ...
                prepare: function() {
                  ...
                  this.view.model.on('change', function() {
                    if (!_.isUndefined(this.get('dicspc')) && this.get('dicspc') !== '') {
                      this.errorModel.clear();
                      self.__internal.buttons[2].element.disabled = false;
                    } else {
                      self.__internal.buttons[2].element.disabled = true;
                      this.errorModel.set('dicspc', gettext('Please select dictionary.'));
                    }
                  });
                },
  ...
  return pgBrowser.Nodes['coll-dictionary'];
});
