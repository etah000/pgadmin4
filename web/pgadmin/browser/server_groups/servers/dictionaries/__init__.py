...
class DictionaryModule(CollectionNodeModule):
    NODE_TYPE = 'dictionary'
    COLLECTION_LABEL = gettext("Dictionaries")

    def __init__(self, import_name, **kwargs):
        super(DictionaryModule, self).__init__(import_name, **kwargs)
...
blueprint = DictionaryModule(__name__)


class DictionaryView(PGChildNodeView):
    ...
    def check_precondition(f):
        ...
        def wrap(*args, **kwargs):
            # Here args[0] will hold self & kwargs will hold gid,sid,dcid
            self = args[0]
            ...
            self.template_path = 'dictionaries/sql/#{0}#'.format(
                self.manager.version
            )
            current_app.logger.debug(
                "Using the template path: %s", self.template_path
            )
            # Allowed ACL on dictionary
            self.acl = ['C']
    ...
    @check_precondition
    def node(self, gid, sid, dcid):
        SQL = render_template(
            "/".join([self.template_path, 'nodes.sql']),
            dcid=dcid, conn=self.conn
        )
        ...
        if len(rset['rows']) == 0:
            return gone(gettext("""Could not find the dictionary."""))
    ...
    @check_precondition
    def nodes(self, gid, sid, dcid=None):
        res = []
        SQL = render_template(
            "/".join([self.template_path, 'nodes.sql']),
            dcid=dcid, conn=self.conn
    ...
    def _formatter(self, data, dcid=None):
        """
        Args:
    ...
    @check_precondition
    def properties(self, gid, sid, dcid):
        SQL = render_template(
            "/".join([self.template_path, 'properties.sql']),
            dcid=dcid, conn=self.conn
        )
        ...
        if len(res['rows']) == 0:
            return gone(
                gettext("""Could not find the dictionary information.""")
            )
        ...
        copy_data = self._formatter(copy_data, dcid)
    ...
    def create(self, gid, sid):
        """
        This function will creates new the dictionary object
        """
        ...
        try:
            ...
            # To fetch the oid of newly created dictionary
            SQL = render_template(
                "/".join([self.template_path, 'alter.sql']),
                dictionary=data['name'], conn=self.conn
            )
    ...
    @check_precondition
    def update(self, gid, sid, dcid):
        """
        This function will update dictionary object
        """
        data = request.form if request.form else json.loads(
            request.data, encoding='utf-8'
        )
        ...
        try:
            SQL, name = self.get_sql(gid, sid, data, dcid)
    ...
    @check_precondition
    def delete(self, gid, sid, dcid=None):
        """
        This function will drop the dictionary object
        """
        if dcid is None:
            data = request.form if request.form else json.loads(
                request.data, encoding='utf-8'
        ...
        try:
            for dcid in data['ids']:
                # Get name for dictionary from dcid
                ...
                if not rset['rows']:
                    return make_json_response(
                        ...
                        info=gettext(
                            'The specified dictionary could not be found.\n'
                        )
                    )
            ...
            return make_json_response(
                success=1,
                info=gettext("Dictionary dropped")
            )
    ...
    @check_precondition
    def msql(self, gid, sid, dcid=None):
        """
    ...
    def get_sql(self, gid, sid, data, dcid=None):
        """
        ...
        if dcid is not None:
            ...
            if len(res['rows']) == 0:
                return gone(
                    gettext("Could not find the dictionary on the server.")
                )

            # Making copy of output for further processing
            old_data = dict(res['rows'][0])
            old_data = self._formatter(old_data, dcid)
            ...
            # If the request for new object which do not have dcid
            SQL = render_template(
                "/".join([self.template_path, 'create.sql']),
                data=data
            )
    ...
    def variable_options(self, gid, sid):
        ...
        if ver >= 90600:
            SQL = render_template(
                "/".join(['dictionaries/sql/9.6_plus', 'variables.sql'])
            )
        else:
            SQL = render_template(
    ...
    @check_precondition
    def statistics(self, gid, sid, dcid=None):
        """
    ...
    @check_precondition
    def dependencies(self, gid, sid, dcid):
        """
    ...
    @check_precondition
    def dependents(self, gid, sid, dcid):
        """
        ...
        Args:
            ...
            dcid: Dictionary ID
        """
        dependents_result = self.get_dependents(self.conn, sid, dcid)
        return ajax_response(
            response=dependents_result,
            status=200
        )

    def get_dependents(self, conn, sid, dcid):
        """
        ...
        for db_row in db_result['rows']:
            ...
            if temp_conn.connected():
                query = render_template(
                    "/".join([self.template_path, 'dependents.sql']),
                    fetch_dependents=True, dcid=dcid
                )
    ...
    @check_precondition
    def move_objects(self, gid, sid, dcid):
        """
        This function moves objects from current dictionary to another
    ...
    @check_precondition
    def move_objects_sql(self, gid, sid, dcid):
        """
...
DictionaryView.register_node_view(blueprint)
