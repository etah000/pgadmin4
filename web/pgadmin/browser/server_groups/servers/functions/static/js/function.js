...
define('pgadmin.node.function', [
    ...
    pgBrowser.Nodes['function'] = pgBrowser.Node.extend({
      ...
      Init: function() {
        ...
        pgBrowser.add_menus([{
          ...
          icon: 'wcTabIcon icon-function', data: {action: 'create'},
          enable: 'can_create_function',
        },{
          ...
          name: 'move_function', node: 'function', module: this,
          applies: ['object', 'context'], callback: 'move_objects',
          category: 'move_function', priority: 5,
      ...
      callbacks: {
        /* Move objects from one function to another */
        move_objects: function(args){
          ...
          var objModel = Backbone.Model.extend({
            idAttribute: 'id',
            defaults: {
              new_fctspc: undefined,
              obj_type: 'all',
              user: undefined,
            },
            schema: [{
              id: 'fctspc', label: gettext('New function'),
              type: 'text', disabled: false, control: 'node-list-by-name',
              node: 'function', select2: {allowClear: false},
              filter: function(o) {
          ...
          if(!Alertify.move_objects_dlg) {
            Alertify.dialog('move_objects_dlg' ,function factory() {
              return {
                main: function() {
                  var title = gettext('Move objects to another function');
                ...
                prepare: function() {
                  ...
                  this.view.model.on('change', function() {
                    if (!_.isUndefined(this.get('fctspc')) && this.get('fctspc') !== '') {
                      this.errorModel.clear();
                      self.__internal.buttons[2].element.disabled = false;
                    } else {
                      self.__internal.buttons[2].element.disabled = true;
                      this.errorModel.set('fctspc', gettext('Please select function.'));
                    }
                  });
                },
  ...
  return pgBrowser.Nodes['coll-function'];
});
