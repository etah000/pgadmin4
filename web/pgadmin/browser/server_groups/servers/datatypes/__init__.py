...
class DatatypeModule(CollectionNodeModule):
    NODE_TYPE = 'datatype'
    COLLECTION_LABEL = gettext("Datatypes")

    def __init__(self, import_name, **kwargs):
        super(DatatypeModule, self).__init__(import_name, **kwargs)
...
blueprint = DatatypeModule(__name__)


class DatatypeView(PGChildNodeView):
    ...
    def check_precondition(f):
        ...
        def wrap(*args, **kwargs):
            # Here args[0] will hold self & kwargs will hold gid,sid,dtid
            self = args[0]
            ...
            self.template_path = 'datatypes/sql/#{0}#'.format(
                self.manager.version
            )
            current_app.logger.debug(
                "Using the template path: %s", self.template_path
            )
            # Allowed ACL on datatype
            self.acl = ['C']
    ...
    @check_precondition
    def node(self, gid, sid, dtid):
        SQL = render_template(
            "/".join([self.template_path, 'nodes.sql']),
            dtid=dtid, conn=self.conn
        )
        ...
        if len(rset['rows']) == 0:
            return gone(gettext("""Could not find the datatype."""))
    ...
    @check_precondition
    def nodes(self, gid, sid, dtid=None):
        res = []
        SQL = render_template(
            "/".join([self.template_path, 'nodes.sql']),
            dtid=dtid, conn=self.conn
    ...
    def _formatter(self, data, dtid=None):
        """
        Args:
    ...
    @check_precondition
    def properties(self, gid, sid, dtid):
        SQL = render_template(
            "/".join([self.template_path, 'properties.sql']),
            dtid=dtid, conn=self.conn
        )
        ...
        if len(res['rows']) == 0:
            return gone(
                gettext("""Could not find the datatype information.""")
            )
        ...
        copy_data = self._formatter(copy_data, dtid)
    ...
    def create(self, gid, sid):
        """
        This function will creates new the datatype object
        """
        ...
        try:
            ...
            # To fetch the oid of newly created datatype
            SQL = render_template(
                "/".join([self.template_path, 'alter.sql']),
                datatype=data['name'], conn=self.conn
            )

            status, dtid = self.conn.execute_scalar(SQL)

            if not status:
                return internal_server_error(errormsg=dtid)
    ...
    @check_precondition
    def update(self, gid, sid, dtid):
        """
        This function will update datatype object
        """
        data = request.form if request.form else json.loads(
            request.data, encoding='utf-8'
        )
        ...
        try:
            SQL, name = self.get_sql(gid, sid, data, dtid)
    ...
    @check_precondition
    def delete(self, gid, sid, dtid=None):
        """
        This function will drop the datatype object
        """
        if dtid is None:
            data = request.form if request.form else json.loads(
                request.data, encoding='utf-8'
        ...
        try:
            for dtid in data['ids']:
                # Get name for datatype from dtid
                ...
                if not rset['rows']:
                    return make_json_response(
                        ...
                        info=gettext(
                            'The specified datatype could not be found.\n'
                        )
                    )
    ...
    @check_precondition
    def msql(self, gid, sid, dtid=None):
        """
    ...
    def get_sql(self, gid, sid, data, dtid=None):
        """
        ...
        if dtid is not None:
            ...
            if len(res['rows']) == 0:
                return gone(
                    gettext("Could not find the datatype on the server.")
                )

            # Making copy of output for further processing
            old_data = dict(res['rows'][0])
            old_data = self._formatter(old_data, dtid)
            ...
            # If the request for new object which do not have dtid
            SQL = render_template(
                "/".join([self.template_path, 'create.sql']),
                data=data
            )
    ...
    def variable_options(self, gid, sid):
        ...
        if ver >= 90600:
            SQL = render_template(
                "/".join(['datatypes/sql/9.6_plus', 'variables.sql'])
            )
        else:
            SQL = render_template(
    ...
    @check_precondition
    def statistics(self, gid, sid, dtid=None):
        """
    ...
    @check_precondition
    def dependencies(self, gid, sid, dtid):
        """
        ...
        Args:
            ...
            dtid: Datatype ID
        """
        dependencies_result = self.get_dependencies(self.conn, dtid)
        return ajax_response(
    ...
    @check_precondition
    def dependents(self, gid, sid, dtid):
        """
        ...
        Args:
            ...
            dtid: Datatype ID
        """
        dependents_result = self.get_dependents(self.conn, sid, dtid)
        return ajax_response(
            response=dependents_result,
            status=200
        )

    def get_dependents(self, conn, sid, dtid):
        """
        ...
        for db_row in db_result['rows']:
            ...
            if temp_conn.connected():
                query = render_template(
                    "/".join([self.template_path, 'dependents.sql']),
                    fetch_dependents=True, dtid=dtid
                )
    ...
    @check_precondition
    def move_objects(self, gid, sid, dtid):
        """
        This function moves objects from current datatype to another
    ...
    @check_precondition
    def move_objects_sql(self, gid, sid, dtid):
        """
...
DatatypeView.register_node_view(blueprint)
