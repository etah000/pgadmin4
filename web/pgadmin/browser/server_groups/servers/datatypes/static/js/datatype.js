...
define('pgadmin.node.datatype', [
  ...
  if (!pgBrowser.Nodes['coll-datatype']) {
    pgBrowser.Nodes['coll-datatype'] =
  ...
  if (!pgBrowser.Nodes['datatype']) {
    pgBrowser.Nodes['datatype'] = pgBrowser.Node.extend({
      parent_type: 'server',
      type: 'datatype',
      sqlAlterHelp: 'sql-alterdatatype.html',
      sqlCreateHelp: 'sql-createdatatype.html',
      ...
      Init: function() {
        ...
        pgBrowser.add_menus([{
          ...
          name: 'create_datatype', node: 'datatype', module: this,
          applies: ['object', 'context'], callback: 'show_obj_properties',
          category: 'create', priority: 4, label: gettext('Datatype...'),
          icon: 'wcTabIcon icon-datatype', data: {action: 'create'},
          enable: 'can_create_datatype',
        },{
          name: 'move_datatype', node: 'datatype', module: this,
          applies: ['object', 'context'], callback: 'move_objects',
          category: 'move_datatype', priority: 5,
      ...
      can_create_datatype: function(node, item) {
      ...
      callbacks: {
        /* Move objects from one datatype to another */
        move_objects: function(args){
          ...
          var objModel = Backbone.Model.extend({
            idAttribute: 'id',
            defaults: {
              new_dtpspc: undefined,
              obj_type: 'all',
              user: undefined,
            },
            schema: [{
              id: 'dtpspc', label: gettext('New datatype'),
              type: 'text', disabled: false, control: 'node-list-by-name',
              node: 'datatype', select2: {allowClear: false},
              filter: function(o) {
              ...
              type: 'text', disabled: false, control: Backform.SqlTabControl.extend({
                ...
                onTabChange: function(obj) {
                  ...
                  if (this.dialog && obj.shown == this.tabIndex) {
                    ...
                    // Add existing datatype
                    args.old_dtpspc = d.label;
          ...
          if(!Alertify.move_objects_dlg) {
            Alertify.dialog('move_objects_dlg' ,function factory() {
              return {
                main: function() {
                  var title = gettext('Move objects to another datatype');
                ...
                prepare: function() {
                  ...
                  this.view.model.on('change', function() {
                    if (!_.isUndefined(this.get('dtpspc')) && this.get('dtpspc') !== '') {
                      this.errorModel.clear();
                      self.__internal.buttons[2].element.disabled = false;
                    } else {
                      self.__internal.buttons[2].element.disabled = true;
                      this.errorModel.set('dtpspc', gettext('Please select datatype.'));
                    }
                  });
                },
      ...
      model: pgBrowser.Node.Model.extend({
        ...
        initialize: function(attrs, args) {
          ...
          if (isNew) {
            ...
            this.set({'case_insensitive': userInfo.name}, {silent: true});
          }
        ...
        schema: [{
          ...
          id: 'is_sys_obj', label: gettext('System datatype?'),
  ...
  return pgBrowser.Nodes['coll-datatype'];
});
