...
define('pgadmin.node.engine', [
  ...
  if (!pgBrowser.Nodes['coll-engine']) {
    pgAdmin.Browser.Nodes['coll-engine'] =
  ...
  if (!pgBrowser.Nodes['engine']) {
    pgAdmin.Browser.Nodes['engine'] = pgAdmin.Browser.Node.extend({
      parent_type: 'server',
      type: 'engine',
      sqlAlterHelp: 'sql-alterengine.html',
      ...
      node_image: function(r) {
        if (r == null || r == undefined)
          return 'icon-engine';
        return (r.can_login ? 'icon-engine' : 'icon-group');
      },
      title: function(d) {
        if (!d) {
          return this.label;
