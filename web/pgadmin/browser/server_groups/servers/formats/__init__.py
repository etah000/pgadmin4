...
class FormatModule(CollectionNodeModule):
    NODE_TYPE = 'format'
    COLLECTION_LABEL = gettext("Formats")

    def __init__(self, import_name, **kwargs):
        super(FormatModule, self).__init__(import_name, **kwargs)
...
blueprint = FormatModule(__name__)


class FormatView(PGChildNodeView):
    ...
    def check_precondition(f):
        ...
        def wrap(*args, **kwargs):
            # Here args[0] will hold self & kwargs will hold gid,sid,fsid
            self = args[0]
            ...
            self.template_path = 'formats/sql/#{0}#'.format(
                self.manager.version
            )
            current_app.logger.debug(
                "Using the template path: %s", self.template_path
            )
            # Allowed ACL on format
            self.acl = ['C']
    ...
    def node(self, gid, sid, fsid):
        ...
        if len(rset['rows']) == 0:
            return gone(gettext("""Could not find the format."""))
    ...
    @check_precondition
    def nodes(self, gid, sid, fsid=None):
        res = []
        SQL = render_template(
            "/".join([self.template_path, 'nodes.sql']),
            fsid=fsid, conn=self.conn
    ...
    def _formatter(self, data, fsid=None):
        """
        Args:
    ...
    @check_precondition
    def properties(self, gid, sid, fsid):
        SQL = render_template(
            "/".join([self.template_path, 'properties.sql']),
            fsid=fsid, conn=self.conn
        )
        ...
        if len(res['rows']) == 0:
            return gone(
                gettext("""Could not find the format information.""")
            )
        ...
        copy_data = self._formatter(copy_data, fsid)
    ...
    def create(self, gid, sid):
        """
        This function will creates new the format object
        """
        ...
        try:
            ...
            # To fetch the oid of newly created format
            SQL = render_template(
                "/".join([self.template_path, 'alter.sql']),
                format=data['name'], conn=self.conn
            )

            status, fsid = self.conn.execute_scalar(SQL)

            if not status:
                return internal_server_error(errormsg=fsid)
    ...
    @check_precondition
    def update(self, gid, sid, fsid):
        """
        This function will update format object
        """
        data = request.form if request.form else json.loads(
            request.data, encoding='utf-8'
        )
        ...
        try:
            SQL, name = self.get_sql(gid, sid, data, fsid)
    ...
    @check_precondition
    def delete(self, gid, sid, fsid=None):
        """
        This function will drop the format object
        """
        if fsid is None:
            data = request.form if request.form else json.loads(
                request.data, encoding='utf-8'
            ...
            for fsid in data['ids']:
                ...
                if not rset['rows']:
                    return make_json_response(
                        ...
                        info=gettext(
                            'The specified format could not be found.\n'
                        )
                    )
    ...
    @check_precondition
    def msql(self, gid, sid, fsid=None):
        """
    ...
    def get_sql(self, gid, sid, data, fsid=None):
        """
        ...
        if fsid is not None:
            ...
            if len(res['rows']) == 0:
                return gone(
                    gettext("Could not find the format on the server.")
                )

            # Making copy of output for further processing
            old_data = dict(res['rows'][0])
            old_data = self._formatter(old_data, fsid)
            ...
            # If the request for new object which do not have fsid
            SQL = render_template(
                "/".join([self.template_path, 'create.sql']),
                data=data
            )
    ...
    def variable_options(self, gid, sid):
        ...
        if ver >= 90600:
            SQL = render_template(
                "/".join(['formats/sql/9.6_plus', 'variables.sql'])
            )
        else:
            SQL = render_template(
    ...
    @check_precondition
    def statistics(self, gid, sid, fsid=None):
        """
    ...
    @check_precondition
    def dependencies(self, gid, sid, fsid):
        """
    ...
    @check_precondition
    def dependents(self, gid, sid, fsid):
        """
        ...
        Args:
            ...
            fsid: Format ID
        """
        dependents_result = self.get_dependents(self.conn, sid, fsid)
        return ajax_response(
            response=dependents_result,
            status=200
        )

    def get_dependents(self, conn, sid, fsid):
        """
        ...
        for db_row in db_result['rows']:
            ...
            if temp_conn.connected():
                query = render_template(
                    "/".join([self.template_path, 'dependents.sql']),
                    fetch_dependents=True, fsid=fsid
                )
    ...
    @check_precondition
    def move_objects(self, gid, sid, fsid):
        """
        This function moves objects from current format to another
    ...
    @check_precondition
    def move_objects_sql(self, gid, sid, fsid):
        """
...
FormatView.register_node_view(blueprint)
