...
define('pgadmin.node.format', [
  ...
  if (!pgBrowser.Nodes['coll-format']) {
    pgBrowser.Nodes['coll-format'] =
  ...
  if (!pgBrowser.Nodes['format']) {
    pgBrowser.Nodes['format'] = pgBrowser.Node.extend({
      parent_type: 'server',
      type: 'format',
      sqlAlterHelp: 'sql-alterformat.html',
      sqlCreateHelp: 'sql-createformat.html',
      ...
      Init: function() {
        ...
        pgBrowser.add_menus([{
          ...
          category: 'create', priority: 4, label: gettext('Format...'),
          icon: 'wcTabIcon icon-format', data: {action: 'create'},
          enable: 'can_create_format',
        },{
          name: 'create_format_on_coll', node: 'coll-format', module: this,
          applies: ['object', 'context'], callback: 'show_obj_properties',
          category: 'create', priority: 4, label: gettext('Format...'),
          icon: 'wcTabIcon icon-format', data: {action: 'create'},
          enable: 'can_create_format',
        },{
          name: 'create_format', node: 'format', module: this,
          applies: ['object', 'context'], callback: 'show_obj_properties',
          category: 'create', priority: 4, label: gettext('Format...'),
          icon: 'wcTabIcon icon-format', data: {action: 'create'},
          enable: 'can_create_format',
        },{
          name: 'move_format', node: 'format', module: this,
          applies: ['object', 'context'], callback: 'move_objects',
          category: 'move_format', priority: 5,
      ...
      callbacks: {
        /* Move objects from one format to another */
        move_objects: function(args){
          ...
          var objModel = Backbone.Model.extend({
            idAttribute: 'id',
            defaults: {
              new_fmtspc: undefined,
              obj_type: 'all',
              user: undefined,
            },
            schema: [{
              id: 'fmtspc', label: gettext('New format'),
              type: 'text', disabled: false, control: 'node-list-by-name',
              node: 'format', select2: {allowClear: false},
              filter: function(o) {
          ...
          if(!Alertify.move_objects_dlg) {
            Alertify.dialog('move_objects_dlg' ,function factory() {
              return {
                main: function() {
                  var title = gettext('Move objects to another format');
                ...
                prepare: function() {
                  ...
                  this.view.model.on('change', function() {
                    if (!_.isUndefined(this.get('fmtspc')) && this.get('fmtspc') !== '') {
                      this.errorModel.clear();
                      self.__internal.buttons[2].element.disabled = false;
                    } else {
                      self.__internal.buttons[2].element.disabled = true;
                      this.errorModel.set('fmtspc', gettext('Please select format.'));
                    }
                  });
                },
      ...
      model: pgBrowser.Node.Model.extend({
        ...
        initialize: function(attrs, args) {
          ...
          if (isNew) {
            ...
            this.set({'is_input': userInfo.name}, {silent: true});
          }
  ...
  return pgBrowser.Nodes['coll-format'];
});
