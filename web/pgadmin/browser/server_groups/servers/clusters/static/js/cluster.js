...
define('pgadmin.node.cluster', [
  ...
  if (!pgBrowser.Nodes['coll-cluster']) {
    pgBrowser.Nodes['coll-cluster'] =
    ...
    pgBrowser.Nodes['cluster'] = pgBrowser.Node.extend({
      ...
      callbacks: {
        ...
        beforeopen: function(item, data) {
          if(!data || data._type != 'cluster' || data.label == 'template0') {
            return false;
          }
  ...
  return pgBrowser.Nodes['coll-cluster'];
});
