...
define('pgadmin.node.shard', [
  ...
  // Extend the browser's collection class for shard collection
  if (!pgBrowser.Nodes['coll-shard']) {
    pgBrowser.Nodes['coll-shard'] =
      pgBrowser.Collection.extend({
        node: 'shard',
        label: gettext('Shards'),
        type: 'coll-shard',
        columns: ['name', 'namespaceowner', 'description'],
      });
  }
  // Extend the browser's node class for shard node
  if (!pgBrowser.Nodes['shard']) {
    pgBrowser.Nodes['shard'] = pgBrowser.Node.extend({
      parent_type: 'cluster',
      type: 'shard',
      sqlAlterHelp: 'sql-altershard.html',
      sqlCreateHelp: 'sql-createshard.html',
