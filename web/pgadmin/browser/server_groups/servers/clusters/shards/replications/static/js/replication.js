...
define('pgadmin.node.replication', [
  ...
  if (!pgBrowser.Nodes['coll-replication']) {
    pgBrowser.Nodes['coll-replication'] =
      pgBrowser.Collection.extend({
        node: 'replication',
        label: gettext('Replications'),
        type: 'coll-replication',
  ...
  if (!pgBrowser.Nodes['replication']) {
    pgBrowser.Nodes['replication'] = SchemaChild.SchemaChildNode.extend({
      parent_type: 'shard',
      type: 'replication',
      ...
      Init: function() {
        ...
        pgBrowser.add_menus([{
          name: 'create_replication_on_coll', node: 'coll-replication', module: this,
          applies: ['object', 'context'], callback: 'show_obj_properties',
          category: 'create', priority: 1, label: gettext('Replication...'),
          icon: 'wcTabIcon icon-replication', data: {action: 'create', check: true},
          enable: 'canCreate',
        },{
          name: 'create_replication', node: 'replication', module: this,
          applies: ['object', 'context'], callback: 'show_obj_properties',
          category: 'create', priority: 1, label: gettext('Replication...'),
          icon: 'wcTabIcon icon-replication', data: {action: 'create', check: true},
          enable: 'canCreate',
        },{
          name: 'create_replication__on_schema', node: 'schema', module: this,
          applies: ['object', 'context'], callback: 'show_obj_properties',
          category: 'create', priority: 4, label: gettext('Replication...'),
          icon: 'wcTabIcon icon-replication', data: {action: 'create', check: false},
          enable: 'canCreate',
        },{
        ...
        pgBrowser.Events.on(
          'pgadmin:browser:node:replication:updated', this.onReplicationUpdated, this
        );
        pgBrowser.Events.on(
      ...
      callbacks: {
        ...
        reset_replication_stats: function(args) {
          var input = args || {},
            obj = this,
            t = pgBrowser.tree,
            i = input.item || t.selected(),
          ...
          Alertify.confirm(
            ...
            gettext('Are you sure you want to reset the statistics for replication "%s"?', d._label),
            function (e) {
              if (e) {
                var data = d;
                $.ajax({
                  ...
                  .done(function(res) {
                    if (res.success == 1) {
                      Alertify.success(res.info);
                      t.removeIcon(i);
                      data.icon = data.is_partitioned ? 'icon-partition': 'icon-replication';
                      t.addIcon(i, {icon: data.icon});
                      t.unload(i);
                      t.setInode(i);
                      t.deselect(i);
  ...
  return pgBrowser.Nodes['replication'];
});
